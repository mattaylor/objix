...
// runtimes resolve `node:assert/strict` via require, and both expose
// `performance` as a global, so no perf_hooks import is needed.
//
...
function report(title, ob) {
  ...
  console.log(title)
  // Copied onto a null prototype so the rows are keyed by operation name.
  // Bun's console.table treats anything carrying Symbol.iterator as array-like
  // and would otherwise number the first column, and objix puts an @@iterator on
  // Object.prototype. A null-prototype object inherits neither. Node's output is
  // unchanged.
  console.table(Object.assign(Object.create(null), tab))
}
const d1 = new Date()
const d2 = new Date()
const testOb = {}
