...
The methods are highly optimised with zero copy operations in most cases. There is however very limited type checking to guard against unwanted side effects. Performance as a result is in many cases signifantly faster than lodash equivalents. (eg `ob.map` is about 25-30% faster than `_.mapValues(ob)` according to simple [benchmarks](bench.js)
